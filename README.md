...
Worker runs are work-conserving: starting from the applicable role, a Worker should continue through eligible work and roles rather than stopping after one task. After material state changes, eligibility should be re-evaluated. A normal run stops only after a complete five-role pass produces no eligible progress. Unchanged blockers must not be retried indefinitely, and Workers should not wait idly for external results when unrelated eligible work exists.

Ownership protection is **target-scoped only**. A live `WORK-CLAIM` protects only the named issue/task. Claiming issue A creates no ownership over issue B, related tasks, dependencies, a phase, a role, the cursor or the project. Conflicting Workers skip only the protected target and continue other eligible work.

Multiple Workers may overlap in time on non-conflicting eligible targets.
