...
const family = path.join(root, 'textures/tiles/dirt');
const manifest = JSON.parse(read('textures/tiles/dirt/dirt_tiles.manifest.json'));
const descriptions = JSON.parse(read('textures/tiles/dirt/dirt_tiles.descriptions.json'));
const config = read('js/config.js');
const renderer = read('js/renderer.js');

assert.strictEqual(manifest.family, 'dirt');
...
for (let row = 0; row < 10; row += 1) {
  for (let col = 0; col < 10; col += 1) {
    ...
    const filename = `dirt_${semantic}_100px.png`;
...
assert.match(config, /dirt: "tiles\/dirt\/dirt_r00_c00_100px\.png"/);
assert.match(config, /dirt: Array\.from\(\{ length: 100 \}/);
assert.match(config, /tiles\/dirt\/dirt_r\$\{String\(row\)/);
assert.match(renderer, /stableTextureVariantIndex/);
assert.match(renderer, /TEXTURE_VARIANT_FILES/);

console.log('WP-102/I02 dirt atlas integration regression: PASS');
