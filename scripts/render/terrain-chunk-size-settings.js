...
function bindControl(select){if(!select)return false;select.value=String(current);if(!select.dataset.terrainChunkSizeBound){select.dataset.terrainChunkSizeBound="true";select.addEventListener("change",()=>set(select.value));}return true;}
function syncControl(){bindControl(document.getElementById("terrainChunkSizeSelect"));}
function syncControlSoon(){requestAnimationFrame(()=>syncControl());}
function bindSettingsOpenSync(){const button=document.getElementById("settingsButton");if(button&&!button.dataset.terrainChunkSizeSyncBound){button.dataset.terrainChunkSizeSyncBound="true";button.addEventListener("click",syncControlSoon);}}
function installControl(){const existing=document.getElementById("terrainChunkSizeSelect");if(existing){bindControl(existing);return;}const heading=document.getElementById("terrainPerformanceHeading"),section=heading?.closest(".settings-subsection");if(!section)return;const label=document.createElement("label");label.className="select-setting";label.htmlFor="terrainChunkSizeSelect";const copy=document.createElement("span");const title=document.createElement("strong");title.textContent="Terrain Chunk Size";const help=document.createElement("small");help.textContent="Partition deterministic terrain into reusable PlayCanvas chunks. 16×16 is the default.";copy.append(title,help);const select=document.createElement("select");select.id="terrainChunkSizeSelect";select.setAttribute("aria-label","Terrain Chunk Size");for(const size of OPTIONS){const option=document.createElement("option");option.value=String(size);option.textContent=size+"×"+size+" tiles"+(size===DEFAULT?" (default)":"");select.appendChild(option);}label.append(copy,select);const firstSetting=section.querySelector(".select-setting, .toggle-setting");section.insertBefore(label,firstSetting||null);bindControl(select);}
function bindPreloadManager(){const api=window.TerrainChunkPreload;if(!api||typeof api.createManager!=="function"||api.__chunkSizeSettingsBound)return false;const original=api.createManager.bind(api);const wrapped={...api,createManager(options={}){const manager=original({...options,chunkSize:current});subscribe(state=>manager?.setChunkSize?.(state.chunkSize));return manager;}};Object.defineProperty(wrapped,"__chunkSizeSettingsBound",{value:true,enumerable:false});window.TerrainChunkPreload=Object.freeze(wrapped);return true;}
...
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",()=>{installControl();bindSettingsOpenSync();syncControl();bindPreloadManager();},{once:true});else{installControl();bindSettingsOpenSync();syncControl();bindPreloadManager();}
window.addEventListener("load",()=>{installControl();bindSettingsOpenSync();syncControl();bindPreloadManager();},{once:true});
window.addEventListener("pageshow",syncControlSoon);
})();
