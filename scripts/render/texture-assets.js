...
async function preloadKeys(keys,{pin=false,deferEvict=false}={}){if(!window.PIXI)throw new Error("PixiJS is not available");const manifest=catalog(),unique=normalizeKeys(keys);if(pin)pinKeys(unique);const missing=unique.filter(key=>!textures.has(cacheKey(key)));await Promise.all(missing.map(async key=>{const texture=await resolveKey(key,manifest),ck=cacheKey(key);textures.set(ck,texture);lastUsed.set(ck,Date.now())}));unique.forEach(touch);if(!deferEvict)evict();return unique.map(key=>textures.get(cacheKey(key)))}
...
async function preloadAll(){const legacy="legacy:all";if(ready&&isRegionPrepared(legacy,Object.keys(catalog())))return stats();if(loadingPromise)return loadingPromise;loadingPromise=(async()=>{const keys=Object.keys(catalog());await preloadKeys(keys,{pin:false,deferEvict:true});preparedRegions.clear();activateRegion(legacy,keys,{wasPrefetched:false});return stats()})();try{return await loadingPromise}finally{loadingPromise=null}}
