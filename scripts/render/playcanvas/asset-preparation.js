...
function create({app,pc,cacheLimit=DEFAULT_LIMIT}={}){
  ...
  let serial=0,hits=0,misses=0,waits=0,evictions=0,loads=0;
  ...
  function stats(){
    ...
    for(const [key,e] of cache){const spec=registry.get(key);if(spec?.type==="container")meshes++;if(spec?.type==="texture")textures++;if(spec?.character)characters++;if(e.pins>0)pinned++;if(e.promise)pending++;}
    return Object.freeze({registered:registry.size,cached:cache.size,meshContainers:meshes,textures,characterAssets:characters,pinned,pending,hits,misses,waits,loads,evictions,cacheLimit,regions:[...regions.keys()]});
  }
