...
function snapshot(){const p=active(),bytes=estimatedTextureBytes(p),capacity=budgetTextureCapacity(p),limit=effectiveCacheLimit(p),loaded=window.TextureAssets?.stats?.().totalResolutionCacheEntryCount??0;return Object.freeze({qualityProfile:p.id,qualityLabel:p.label,sourceResolution:SOURCE,runtimeResolution:p.runtimeResolution,defaultProfile:DEFAULT,options:OPTIONS.slice(),textureBudgetMB:p.textureBudgetMB,estimatedTextureBytes:bytes,budgetTextureCapacity:capacity,effectiveCacheLimit:limit,estimatedLoadedTextureMB:Number((loaded*bytes/1048576).toFixed(3)),budgetUtilization:Number((loaded*bytes/(p.textureBudgetMB*1048576)).toFixed(4)),budgetEnforcementCount,anisotropy:p.anisotropy,auxiliaryMaps:p.auxiliaryMaps,detailMaps:p.detailMaps,cacheSignature:"material-quality@"+p.id+"-"+p.runtimeResolution+"px",revision})}
...
current=read();
window.RuntimeTextureResolution=Object.freeze({SOURCE_RESOLUTION:SOURCE,DEFAULT_RESOLUTION:PROFILES[DEFAULT].runtimeResolution,OPTIONS:Object.freeze(OPTIONS.map(id=>PROFILES[id].runtimeResolution)),PROFILES,DEFAULT_PROFILE:DEFAULT,normalizeResolution:value=>PROFILES[profileForResolution(value)].runtimeResolution,get:()=>active().runtimeResolution,set:setResolution,snapshot,cacheSignature:()=>snapshot().cacheSignature,getProfile:()=>current,setProfile:write,profile:active,enforceBudget,effectiveCacheLimit,budgetTextureCapacity,estimatedTextureBytes});
