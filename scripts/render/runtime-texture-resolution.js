...
function write(value){const next=normalize(value);if(next===current){syncSettingsControl();return snapshot()}current=next;try{localStorage.setItem(STORAGE_KEY,current);localStorage.removeItem(LEGACY_KEY)}catch(_){}revision++;enforceBudget();syncSettingsControl();const state=snapshot();window.dispatchEvent(new CustomEvent("advisor:texture-resolution-change",{detail:state}));window.dispatchEvent(new CustomEvent("advisor:texture-quality-change",{detail:state}));requestPreparedPresentationRefresh();return state}
...
function syncSettingsControl(){const select=document.getElementById("textureQualityProfile");if(select)select.value=current}
