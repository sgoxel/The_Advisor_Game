...
async function waitForRuntimeDestinationPaint(maxFrames=12){
...
function navigateCameraTo(target,navigationMeta=null,{forceGate=false}={}){
  ...
  return enqueueCameraTransition(async()=>{
    ...
    try{
      ...
      const paint=await waitForRuntimeDestinationPaint(12);
      ...
      // Superseded camera intents are expected coalescing, not loading errors.
      // The newest queued target owns the gate and status presentation.
      if(intent!==runtimeAreaIntentSerial)return null;
...
function renderCountryProfileProof(profileIndex=0){
  if(typeof window.CountryProfile?.renderDebugPanel!=="function")return null;
  const campaign=SeedSystem.getCampaign();
  const seed=campaign?campaign.seed:SeedSystem.getSettings().seed;
...
function renderCountryRelationsProof(relationIndex=0){
  if(typeof window.CountryRelations?.renderDebugPanel!=="function")return null;
  const campaign=SeedSystem.getCampaign();
  const seed=campaign?campaign.seed:SeedSystem.getSettings().seed;
...
function renderSettlementBuildingCatalogProof(index=0){
  if(typeof window.SettlementBuildingCatalog?.renderDebugPanel!=="function")return null;
  const campaign=SeedSystem.getCampaign();
  const seed=campaign?campaign.seed:SeedSystem.getSettings().seed;
  return window.SettlementBuildingCatalog.renderDebugPanel(seed,index,document.getElementById("settlementBuildingCatalogProof"));
}
function renderWorldStateProof(){
  if(typeof window.WorldState?.renderDebugPanel!=="function")return null;
  const campaign=SeedSystem.getCampaign();
  const seed=campaign?campaign.seed:SeedSystem.getSettings().seed;
...
function renderEventSchedulerProof(){
  if(typeof window.EventScheduler?.renderDebugPanel!=="function")return null;
  const campaign=SeedSystem.getCampaign();
  const seed=campaign?campaign.seed:SeedSystem.getSettings().seed;
...
function renderRegionalSettlementSimulationProof(){
  if(typeof window.RegionalSettlementSimulation?.renderDebugPanel!=="function")return null;
  const campaign=SeedSystem.getCampaign();
  const seed=campaign?campaign.seed:SeedSystem.getSettings().seed;
...
window.AppUI=Object.freeze({
  ...
  refreshSettlementBuildingCatalog:(index=0)=>renderSettlementBuildingCatalogProof(index),
