...
function makeCloudTexture(){
  ...
  for(let y=0;y<source.height;y++)for(let x=0;x<source.width;x++){
    ...
    const alpha=Math.round(clamp((field-.10)*300,0,148));const i=(y*source.width+x)*4;
...
function buildAmbientMotion(surfaceMesh){
  ...
  material.name="SeededCloudLayer";material.diffuse.set(.95,.98,1);material.diffuseMap=texture;material.opacityMap=texture;material.opacityMapChannel="a";material.opacity=.76;material.blendType=pc.BLEND_NORMAL;material.depthWrite=false;material.cull=pc.CULLFACE_BACK;material.useLighting=false;material.update();
...
function updateAmbientMotion(dt){
  ...
  const started=performance.now();ambientMotion.cloudYawDegrees=(ambientMotion.cloudYawDegrees+Math.min(.12,Math.max(0,Number(dt)||0))*3.6)%360;
