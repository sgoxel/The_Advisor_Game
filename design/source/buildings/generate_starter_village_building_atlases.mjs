...
function floorTexture(buf,ox,oy,s,salt=0){const floor=shade(s.wall,30),joint=shade(s.wall,12),light=shade(s.wall,42);rect(buf,ox+4,oy+4,248,248,floor);for(let y=oy+20;y<oy+252;y+=34)line(buf,ox+8,y,ox+248,y,joint,2);for(let x=ox+18;x<ox+252;x+=48){const offset=((x/CELL+salt)%2)*17;for(let y=oy+20+offset;y<oy+252;y+=68)line(buf,x,y,x,Math.min(oy+252,y+34),light,2);}}
