...
(function installStarterVillageInteriors(global) {
  ...
  function render(){const canvas=ensureOverlay(),world=Game.State?.world,player=world?.player;if(!canvas||!world||!player)return;const width=Math.max(1,canvas.clientWidth||Game.State?.dom?.canvas?.clientWidth||1),height=Math.max(1,canvas.clientHeight||Game.State?.dom?.canvas?.clientHeight||1),dpr=Math.max(1,global.devicePixelRatio||1);if(canvas.width!==Math.round(width*dpr)||canvas.height!==Math.round(height*dpr)){canvas.width=Math.round(width*dpr);canvas.height=Math.round(height*dpr);}const ctx=canvas.getContext('2d');if(!ctx)return;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,width,height);const active=interiorAt(player.row,player.col),exterior=document.getElementById('starterVillageExteriorOverlay');if(!active){canvas.dataset.activeBuildingId='';if(exterior)exterior.style.opacity='1';return;}canvas.dataset.activeBuildingId=active.buildingId;if(exterior)exterior.style.opacity='0.24';const footprint=polygonForRect(active.footprint);if(footprint)drawPolygon(ctx,footprint,'rgba(93,75,54,.82)','rgba(236,217,178,.86)');for(const room of active.rooms){const points=polygonForRect({row:room.row,col:room.col,width:room.width,height:room.height});if(points)drawPolygon(ctx,points,'rgba(177,143,91,.18)','rgba(61,48,38,.82)');}const door=project(active.door.row+.5,active.door.col+.5);if(door){ctx.fillStyle='#e8c981';ctx.beginPath();ctx.arc(door.x,door.y,4,0,Math.PI*2);ctx.fill();}}
  ...
  if(typeof document!=='undefined'){if(document.readyState==='loading')global.addEventListener('DOMContentLoaded',install,{once:true});else install();global.setInterval?.(()=>materialize(Game.State?.world),POLL_MS);}
