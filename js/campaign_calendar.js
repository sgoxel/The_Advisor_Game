...
(function installCampaignCalendar() {
  ...
  function currentGameMinutes() {
    ...
    if (!snapshot || snapshot.authority !== AUTHORITY || !Number.isFinite(snapshot.totalGameMinutes)) throw new Error('CampaignCalendar requires authoritative GameTime.');
    return Number(snapshot.totalGameMinutes);
  }

  function civilParts(timestampMs, timezoneOffsetMinutes) {
    const date = new Date(timestampMs - timezoneOffsetMinutes * 60000);
    return { year: date.getUTCFullYear(), month: date.getUTCMonth() + 1, dayOfMonth: date.getUTCDate(), hour: date.getUTCHours(), minute: date.getUTCMinutes() };
  }
  ...
  function ensureState() {
    ...
    if (!existing || existing.version !== VERSION || existing.authority !== AUTHORITY) {
      world.campaignCalendar = { version: VERSION, authority: AUTHORITY, acceptedRealTimestampMs: null, originRealTimestampMs: null, originTimezoneOffsetMinutes: null, fantasyOriginUtcMs: null, originGameMinutes: null, totalGameMinutes: total, catchUpOperations: 0 };
    } else {
  ...
  function dateParts(totalGameMinutes, state = ensureState()) {
    ...
    const dateMs = hasOrigin ? state.fantasyOriginUtcMs + Math.floor(total - state.originGameMinutes) * 60000 : LEGACY_CAMPAIGN_EPOCH_UTC_MS + dayIndex * 86400000 + minuteOfDay * 60000;
    const date = new Date(dateMs);
    return { dayIndex, year: date.getUTCFullYear(), month: date.getUTCMonth() + 1, dayOfMonth: date.getUTCDate(), hour: date.getUTCHours(), minute: date.getUTCMinutes(), isoDate: `${String(date.getUTCFullYear()).padStart(4, '0')}-${String(date.getUTCMonth() + 1).padStart(2, '0')}-${String(date.getUTCDate()).padStart(2, '0')}` };
  }
  ...
  function capture() {
    ...
    return deepFreeze({ version: VERSION, authority: AUTHORITY, acceptedRealTimestampMs: state.acceptedRealTimestampMs, originRealTimestampMs: state.originRealTimestampMs, originTimezoneOffsetMinutes: state.originTimezoneOffsetMinutes, fantasyOriginUtcMs: state.fantasyOriginUtcMs, originGameMinutes: state.originGameMinutes, totalGameMinutes: Number(total.toFixed(6)), catchUpOperations: Number(state.catchUpOperations || 0), calendar: dateParts(total, state) });
  }
  ...
  function reconcileResume(observedRealTimestampMs = Date.now(), timezoneOffsetMinutes = new Date(observedRealTimestampMs).getTimezoneOffset()) {
    ...
    if (observed < accepted) return deepFreeze({ ok: false, code: 'BACKWARD_REAL_CLOCK', acceptedRealTimestampMs: accepted, observedRealTimestampMs: observed, advancedGameMinutes: 0 });
    ...
    return deepFreeze({ ok: true, initialized: false, elapsedRealMilliseconds, advancedGameMinutes: Number(advancedGameMinutes.toFixed(6)), elapsedGameDays: Number((advancedGameMinutes / GAME_MINUTES_PER_DAY).toFixed(6)), operations: advancedGameMinutes > 0 ? 1 : 0, fullDetailReplayTicks: 0, materializedOffscreenRegions: 0, state: capture() });
  }
  ...
  function validateWrappedSave(input) {
    let envelope = input;
    if (typeof input === 'string') { try { envelope = JSON.parse(input); } catch (_error) { return deepFreeze({ ok: false, code: 'INVALID_JSON' }); } }
  ...
  function serializeSaveAt(observedRealTimestampMs, candidate, deltaCandidate, timezoneOffsetMinutes = 0) { return JSON.stringify(createWrappedEnvelope(candidate, deltaCandidate, observedRealTimestampMs, timezoneOffsetMinutes)); }
