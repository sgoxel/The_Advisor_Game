...
(function installProtagonistRoutine(global) {
  ...
  const VERSION = 'r04-protagonist-routine-v2-location-gated';
  const REFRESH_MS = 1000;
  const ROLE_ALIASES = Object.freeze({
    peasant: 'farmer', villager: 'default', farmer: 'farmer', farmhand: 'farmer',
    herder: 'herder', shepherd: 'herder', blacksmith: 'blacksmith', smith: 'blacksmith',
    innkeeper: 'innkeeper', tavernkeeper: 'innkeeper', baker: 'baker', trader: 'trader',
    merchant: 'merchant', guard: 'guard'
  });
  ...
  function anchorPoint(building) {
    ...
    if (Number.isFinite(entranceRow) && Number.isFinite(entranceCol)) return freeze({ row: entranceRow, col: entranceCol, source: 'entrance' });
    const row = Number(building.footprint?.row), col = Number(building.footprint?.col);
    const height = Number(building.footprint?.height), width = Number(building.footprint?.width);
    if ([row, col, height, width].every(Number.isFinite)) return freeze({ row: row + Math.max(0, height - 1) / 2, col: col + Math.max(0, width - 1) / 2, source: 'footprint-center' });
    return null;
  }
  ...
  function scheduleState(totalGameMinutesInput = null, profileInput = currentProfile()) {
    ...
    const totalGameMinutes = totalGameMinutesInput === null ? Game.GameTime?.capture?.()?.totalGameMinutes : Number(totalGameMinutesInput);
    ...
    const gate = Game.NPCLife?.activityLocationState?.(player, npcSchedule, { world: Game.State?.world, target: activeAnchor?.point }) || null;
    return freeze({
      version: VERSION,
      ...
      activity: gate?.activeActivity || npcSchedule.activity,
      intendedActivity: gate?.intendedActivity || npcSchedule.activity,
  ...
  function buildRoutineOpportunity(contextInput = {}) {
    ...
    return freeze({
      ...
      source: 'protagonist-resident-routine', actorId,
      goalType: state.anchor === 'work' ? 'daily-work' : state.anchor === 'social' ? 'daily-social' : 'daily-home',
      actionType: 'move', targetRef: state.targetBuildingId, locationRef: text(contextInput.locationRef) || null,
      priority: -100, urgency: 5, distance: 0,
  ...
  function stop() { if (timer !== null && typeof global.clearInterval === 'function') global.clearInterval(timer); timer = null; }

  Game.ProtagonistRoutine = Object.freeze({ version: VERSION, authority: 'character-routine', routineRole, resolveAnchors, scheduleState, refresh, buildRoutineOpportunity, start, stop });
  if (global.document) {
