...
(function installStaticTileCompositor(global) {
  ...
  function mainRoadVisual(semantic) {
    ...
    if (semantic.orientation === 'horizontal') {
      if (membership.longitudinalRole === 'start' || membership.longitudRole === 'end') {
  ...
  function objectCells() {
    ...
    for (const descriptor of descriptors) {
      ...
      if (occupied.length > 1) {
        // Multi-tile objects require explicit tile-local semantic parts. A single giant sprite
        // is intentionally not stretched across the footprint under the Admin composition rule.
        continue;
      }
  ...
  async function composeNow(reason = 'manual') {
    ...
    try {
      ...
      if (shouldFull) {
        // Restore authoritative terrain/base first so removed static overlays cannot leave ghosts.
        backgroundCtx.clearRect(0, 0, background.width, background.height);
        backgroundCtx.drawImage(baseSnapshot, 0, 0);
        tileCache.clear();
      } else {
        // Restore only explicitly invalidated tiles from the authoritative base before rebuilding.
