...
def _run_scenario_step(driver, scenario: str, frame_index: int, base_width: int, base_height: int) -> str:
    ...
    if scenario == "static" or (
        frame_index == 0 and
        scenario not in {"wp-s004-001","wp-s004-002","wp-s004-003","wp-s004-004","wp-s004-004-001","wp-s004-005","wp-s005-001","wp-s005-002","wp-s005-003","wp-s005-004","wp-s005-005","wp-s006-001","wp-s006-002","wp-s006-003","wp-s006-004","wp-s006-005","wp-s006-006","wp-s007-001","wp-s007-002","wp-s007-003","wp-s003-008-002-001","wp-s003-010-004"}
    ):
        return "initial"
...
def validate_scenario_frames(scenario: str, frames: list[dict]) -> None:
    if scenario == "wp-s003-010-003-002":
        ...
        for index,stage in enumerate(dense, start=1):
            if stage.get("ready") is not True or stage.get("version") != "planet-ground-static-v2":
            ...
            if index>=3 and str(local.get("level") or "") in {"regional-overview","regional-detail","district"} and int(local.get("textureSize") or 0)>128:
                raise RuntimeError(f"Broad LOD texture exceeds 128px streaming budget in frame {index}: {local}")
        ...
        if int(political.get("countryCellSize") or 0) < 131072:
        ...
        if float(political.get("countryRegionLinearRatio") or 0) < 8:
